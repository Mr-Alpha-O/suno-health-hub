...
import { products, productCategories, waLink } from "@/lib/site";
          ...
          {list.map((p) => (
            <div key={p.slug} className="group bg-white rounded-2xl border border-border shadow-soft hover:shadow-elegant overflow-hidden transition-smooth hover:-translate-y-1">
              <Link to="/store/$slug" params={{ slug: p.slug }} className="block aspect-square overflow-hidden bg-secondary/30">
                <img src={p.image} alt={p.name} width={800} height={800} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-smooth" />
              </Link>
              <div className="p-5">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 text-center">
                  <div className="bg-secondary/60 rounded-lg py-2">
                    ...
                    <div className="font-extrabold text-primary text-sm">{p.buy.toLocaleString("ar-EG")} ج.م</div>
                  </div>
                  <div className="bg-secondary/60 rounded-lg py-2">
                    <div className="text-[10px] text-muted-foreground">سعر الإيجار/يوم</div>
                    <div className="font-extrabold text-primary text-sm">{p.rent.toLocaleString("ar-EG")} ج.م</div>
                  </div>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-2">
                  <a href={waLink(`أرغب في شراء: ${p.name}`)} target="_blank" rel="noopener" className="inline-flex items-center justify-center gap-1.5 bg-gradient-hero text-primary-foreground py-2 rounded-lg text-xs font-bold hover:shadow-elegant transition-smooth">
                    <ShoppingCart className="h-3.5 w-3.5" /> شراء
                  </a>
                  <a href={waLink(`أرغب في إيجار: ${p.name}`)} target="_blank" rel="noopener" className="inline-flex items-center justify-center gap-1.5 bg-white text-primary border border-primary/30 py-2 rounded-lg text-xs font-bold hover:bg-secondary transition-smooth">
