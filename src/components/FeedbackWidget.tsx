...
export function FeedbackWidget({ open, onOpenChange }: Props) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setDone(true);
      setTimeout(() => { onOpenChange(false); setDone(false); setRating(null); setName(""); setComment(""); setRequested(""); }, 1800);
    } catch (err: any) {
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-black/40 flex items-end sm:items-center justify-center p-3" onClick={() => onOpenChange(false)} dir="rtl">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-elegant p-5" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => onOpenChange(false)} aria-label="إغلاق" className="text-muted-foreground hover:text-foreground"><X className="h-5 w-5" /></button>
        </div>
        {done ? (
